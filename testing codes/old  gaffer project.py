...
def prjFun():
    ...
    #Adding list box
    Database()
    cur.execute("SELECT PROJ_CODE FROM WORKON,MEMBERS,TEAM WHERE MEM_ID=M_ID AND TEAM_CODE=T_CODE AND USERNAME=?",(CAP_USERNAME.get(),))
    row=cur.fetchall()
...
def memFun():
    ...
    cap_member_delete_button=ttk.Button(allMemberWin,text='Delete**',command=lambda:functionOne(captainDashboard)) #not done: learn to delete from maam gave sql
