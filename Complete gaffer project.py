...
cur.execute("CREATE TABLE IF NOT EXISTS 'WORKON' (PROGRESSBAR INTEGER DEFAULT 20 CHECK(PROGRESSBAR>=-20 AND PROGRESSBAR <=120),MEM_ID CHAR(10),SUBMIT_DATE DATE DEFAULT '0000-00-00', PROJ_CODE INTEGER,PROGRESS INTEGER DEFAULT 0, CONSTRAINT FK5 FOREIGN KEY (MEM_ID) REFERENCES MEMBERS (M_ID) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT FK6 FOREIGN KEY (PROJ_CODE) REFERENCES PROJECT (P_CODE) ON DELETE CASCADE ON UPDATE CASCADE)")
...
def addMemberWin():
    ...
    def insertMember():
        ...
        #print(str(textinn.get()))
        Database()
        cur.execute("INSERT INTO 'MEMBERS' (M_ID,M_NAME,CELL,COUNTRY,START_DATE,TEAM_CODE,ROLE_ID) VALUES('%s','%s','%s','%s','%s','%d','%d')"%(TEMP1,TEMP2,TEMP4,TEMP3,STARTING_DATE,TEAM_CODE,TEMP7))
...
def addProjectWin():
    ...
    #selecting no of members
    multiple_mem_combobox=ttk.Combobox(root2)
    multiple_mem_combobox['values']=(1,2,3,4,5,6)
    multiple_mem_combobox.current(0)
    
    multiple_mem_combobox.grid(row=18,column=2)
    ...
    def assignAnotherMem(widget,TEMP,count): #when ADD NEXT button is clicked
        global PROJECT_MEMID
        widget.destroy()
        ...
        if(count>=TEMP):
            print("error")
            messagebox.showwarning("Warning","Warning! Assignment Exceeeded")
        else:
            assignPrj(count)
    ...
    def assignPrj(count):
        ...
        root3=Toplevel()
        root3.geometry('320x320')
        new_prj_assigned_member_id_label=ttk.Label(root3,text="Member Id",font=("Calibri",12))
        new_prj_assigned_member_id_entry=ttk.Entry(root3,textvariable=PROJECT_MEMID,font=("Calibri",12))
            
        new_prj_assigned_member_id_label.grid(row=1,column=2,sticky=W,pady=5,padx=5)
        new_prj_assigned_member_id_entry.grid(row=1,column=3,pady=5,sticky=E)
            
        prj_submit_btn=ttk.Button(root3,text='ADD',command=updateMemWorkon)
        prj_submit_btn.grid(row=2,column=2)
        
        prj_submit_btn=ttk.Button(root3,text='ADD NEXT',command=lambda: assignAnotherMem(root3,TEMP9,count)) #used Lambda: here because without using it i wasn't able to destroy root3 and pass those parameters in assignAnotherMem funciton
        prj_submit_btn.grid(row=2,column=3)
        
        #new_prj_assigned_member_id_entry.delete(0,END)
            
        
    def insertPrj(count,window):
        global TEMP9,TEMP1
        ...
        #TEMP8=PROJECT_MEMID.get()
        TEMP9=int(multiple_mem_combobox.get())
        print(TEMP9)
        if(TEMP6<TEMP5):
            print("due date error")
            window.destroy()
            messagebox.showwarning("Warning!","Use correct starting or due date")
            addProjectWin()
        ...
        if(int(TEMP8)!=0):
            cur.execute("INSERT INTO 'CLIENT_MOBILE' (MOBILE,CUSTOMER_ID) VALUES('%s','%s')"%(TEMP8,TEMP1))
            """
    count=0  #this is for ensuring that fix no of member are only added to the project (it is used in assignAnotherMem function) 
    prj_submit_btn=ttk.Button(root2,text='SUBMIIT',command=lambda: insertPrj(count,root2))
...
captain_img=PhotoImage(file="images\\captain.png")
captain_img_label=ttk.Label(loginPage,image=captain_img)
...
captain_id_speech_button.grid(row=2,column=2)


captain_img_label.grid(row=1,column=0,columnspan=2)
#captain_label.grid(row=0,column=0,columnspan=2,pady=10)
captain_username_label.grid(row=2,column=0,sticky=W,padx=2)
captain_pwd_label.grid(row=3,column=0,sticky=W,padx=2)
captain_username_entry.grid(row=2,column=1,sticky=W,pady=3)
captain_pwd_entry.grid(row=3,column=1,sticky=W,pady=3)
captain_checkbutton=ttk.Checkbutton(loginPage,text="Keep me logged in")



"""==========================================Verifying Captain login==================================================="""
def verifyCapLogin():
    global TEAM_CODE
    TEAM_CODE=IntVar()
    ...
    Database()
    cur.execute("SELECT USERNAME,PWD,T_CODE FROM TEAM") #select query to retrives all rows from TEAM table
...
team_img=PhotoImage(file="images\\teammembers.png")
team_img_label=ttk.Label(loginPage,image=team_img)
...
team_img_label.grid(row=6,column=0,columnspan=2)
...
"""=================================FUNCTION FOR INCREASING AND DECREASING PROGRESSBAR==============================================="""

def decreaseProgressbar(data):
    if(data>0): #data of progressbar shouldnot go less then 0 to avoid check constraint error
        Database()
        decrease=int(data)-20
        cur.execute("UPDATE 'WORKON' SET PROGRESSBAR=? WHERE MEM_ID=? AND PROJ_CODE=?",(decrease,MEMBER_ID.get(),temp[0],))
        db.commit()
    else:
        print("Error! Your Progress is already 0")
        messagebox.showwarning("Warning!"," Your Progress is already 0")
    
def increaseProgressbar(data):
    if(data<100): #data of progressbar shouldnot go beyond 100 to avoid check constraint error
        Database()
        increase=int(data)+20
        cur.execute("UPDATE 'WORKON' SET PROGRESSBAR=? WHERE MEM_ID=? AND PROJ_CODE=?",(increase,MEMBER_ID.get(),temp[0],))
        db.commit()
    else:
        print("Error!Your Progress is already 100")
        messagebox.showwarning("Warning!"," Your Progress is already 100")
...
def memProgressbar(value):
    ...
    bar=Progressbar(root13,length=100,orient='vertical')
    bar['value']=value
    bar.grid(column=2,row=0,sticky=N+S+E+W)
    
    
    done_btn=ttk.Button(root13,text='Done',command=root13.destroy)
    done_btn.grid(row=1,column=2)
    decrease_btn=ttk.Button(root13,text='Decrease',command=lambda: decreaseProgressbar(value)) 
    Button(root13,text='Reduce',command=decreaseProgressbar(value)) 
    
    increase_btn=ttk.Button(root13,text='Increase',command=lambda: increaseProgressbar(value)) 
...
def verifiedMember():
    ...
    """members haru le afno work complete gare paxi submit button click garyo bhane you 
        function call vayera WORKON table ko Submit_Date ma Today's date update hunxa"""
    def updateSubmitDate():
        global temp
        clicked_item= member_home_listbox.curselection()
        ...
        Database()
        cur.execute("SELECT SUBMIT_DATE FROM WORKON WHERE MEM_ID=? AND PROJ_CODE=?",(MEMBER_ID.get(),temp[0],)) #select query to retrives all rows from TEAM table
        row1=cur.fetchall()
        for data in row1:
            print(data[0])
            
            
        if(data[0]!="0000-00-00"): #initially default value "0000-00-00" haldeko xu create table garda so comparison garna sajilo vayo
            messagebox.showerror("ERROR","You have already submitted this project!")
        else:
            
            Database()
            cur.execute("UPDATE WORKON SET SUBMIT_DATE=? WHERE MEM_ID=? AND PROJ_CODE=?",(submit_date,MEMBER_ID.get(),temp[0],)) 
            db.commit()
            messagebox.showinfo("Submitted","Submition Complete")
            Database()
            cur.execute("SELECT DISTINCT(DUE_DATE) FROM PROJECT,WORKON WHERE P_CODE=PROJ_CODE AND P_CODE=?",(temp[0],)) #select query to retrives all rows from TEAM table
            row2=cur.fetchall()
            for data in row2:
                print(data[0])
            
            print(submit_date.day-int(data[0].split("-")[2]))
            due_month=int(data[0].split("-")[1])
            due_day=int(data[0].split("-")[2])
            submit_month=int(submit_date.month)
            submit_day=int(submit_date.day)
            print(due_month)
            print(due_day)
            print(submit_month)
            print(submit_day)

            """=================================================CREDITS CALCULATION (PROGRESS CALCULATION)===================================================="""
            
            if(due_month==submit_month and due_day<submit_day):
                day_count=submit_day-due_day
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",((100-(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} days late so 100 - {}".format(day_count,(day_count*3)))
            elif(due_month==submit_month and due_day>submit_day):
                day_count=due_day-submit_day
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",((100+(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} days fast so 100 + {}".format(day_count,(day_count*3)))
                
            
            elif(due_month<submit_month and due_day==submit_day):
                month_count=submit_month-due_month
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",((100-(month_count*10)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} month late so 100 - {}".format(month_count,(month_count*10)))
            elif(due_month>submit_month and due_day==submit_day):
                month_count=due_month-submit_month
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",((100+(month_count*10)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} month fast so 100 + {}".format(month_count,(month_count*10)))
                
            elif(due_month>submit_month and due_day>submit_day):
                day_count=due_day-submit_day
                month_count=due_month-submit_month
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",(100+((month_count*10)+(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} month {} days fast so 100+({} + {})" .format(month_count,day_count,(month_count*10),(day_count*3)))
              
            elif(due_month<submit_month and due_day<submit_day):
                day_count=submit_day-due_day #yesma kati din dhilo submit garyo find garne ho so submit-due
                month_count=submit_month-due_month
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",(100-((month_count*10)+(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submited {} month {} days late so 100-({} + {})".format(month_count,day_count,(month_count*10),(day_count*3)))
                ...
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",(100+((month_count*10)+(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit()
                print("you submitted {} month {} days fast so 100+({} + {})" .format(month_count,day_count,(month_count*10),(day_count*3)))
                
            elif(due_month<submit_month and due_day>submit_day):
                month_count=submit_month-due_month
                month_count=month_count-1
                submit_day=submit_day+30
                day_count=submit_day-due_day
                Database()
                cur.execute("UPDATE WORKON SET PROGRESS=? WHERE MEM_ID=? AND PROJ_CODE=?",(100+((month_count*10)+(day_count*3)),MEMBER_ID.get(),temp[0],)) 
                db.commit() 
                print("you submitted {} month {} days late so 100-({} + {})" .format(month_count,day_count,(month_count*10),(day_count*3)))
                
            else:
                print("you submited on the exact date %s so you have 100" % data[0])
    ...
    member_home_update_progress_button=ttk.Button(memberDashboard,text='Update Progress',command=updateProgress)
    member_home_update_progress_button.grid(row=7,column=5,padx=100)
    
    member_home_update_progress_button=ttk.Button(memberDashboard,text='Submit',command=updateSubmitDate)
    member_home_update_progress_button.grid(row=8,column=5,padx=100)
...
def prjFun():
    ...
    def prjBudget():
        ...
        Database()
        print(TEAM_CODE)
        cur.execute("SELECT DISTINCT(P_NAME),BUDGET FROM PROJECT,WORKON,MEMBERS,TEAM WHERE P_CODE=PROJ_CODE AND MEM_ID=M_ID AND TEAM_CODE=T_CODE AND T_CODE=?",(TEAM_CODE,)) #retrives all rows from TEAM table
        row=cur.fetchall() #retrives all rows from TEAM table into row variable

        for data in row:
            pname_list.append(data[0]) #inserting all the progress of the members in each project to progress_list
            budget_list.append(data[1])  #inserting all the project name into pname_list
        ...
        plt.bar(left, height, tick_label = tick_label, width=0.2, color = ['green', 'pink','blue','yellow','purple'])
...
def memFun():
    ...
    def memProgressBarChart():
        ...
        # plotting a bar chart
        plt.bar(left, progress_list, tick_label = tick_label, width=0.2, color = ['green', 'pink','blue','yellow','purple'])
    ...
    cap_member_delete_button=ttk.Button(allMemberWin,text='Delete**',command=delMem) #not done: learn to delete from maam gave sql
...
def clientFun():
    ...
    def clientBarchart():
        ...
        Database()
        print(TEAM_CODE)
        cur.execute("SELECT C_ID,P_NAME FROM CLIENT,PROJECT,WORKON,MEMBERS,TEAM WHERE CUST_ID=C_ID AND P_CODE=PROJ_CODE AND MEM_ID=M_ID AND TEAM_CODE=T_CODE AND T_CODE=? GROUP BY P_CODE",(TEAM_CODE,)) #retrives all rows from TEAM table
        row=cur.fetchall() #retrives all rows from TEAM table into row variable

        for data in row:
            client_id_list.append(data[0]) #inserting all the progress of the members in each project to progress_list
            projects_name_list.append(data[1])  #inserting all the project name into pname_list
        ...
        for i in range(len(client_id_list)): #how many projects did member do- getting length according to that we have to assign X-VALUES of BAR
            left.append(i) # X-VALUES - to be on LEFT side of the BAR #different members may be assigned to different no. of projects so to get specific X-VALUES
        
        height= client_id_list   #passing HEIGHTS of BARS
        print(height)
        tick_label = projects_name_list #passing LABELS for BARS
        print(tick_label)
    ...
    cap_client_piechart_button=ttk.Button(allClientWin,text='Bar Chart',command=clientBarchart)#not done: first complete all then PIE-CHART
...
project_img=PhotoImage(file="images\\project1.png")
project_img_label=ttk.Label(captainDashboard,image=project_img)

member_img=PhotoImage(file="images\\member.png")
member_img_label=ttk.Label(captainDashboard,image=member_img)

client_img=PhotoImage(file="images\\client.png")
...
duedate_img=PhotoImage(file="images\\due date.png")
